...
def advancedPursuit(waypoint,traj,reverse = False):
  ...
  while (target_distance > waypoint_tol):
    ...
    if (reverse == True):
      ai = np.sign(-target_speed - prev_speed) * max(-max_acc,min(max_acc,abs(Kp * (-target_speed - prev_speed))))
    else:
      ai = np.sign(target_speed - prev_speed) * max(-4,min(4,abs(Kp * (target_speed - prev_speed))))
    ...
    target_ind = pursuit.calc_target_index(vstate,traj[0],traj[1],new_Lf = Lfc, new_k = k)
...
def plot2dtraj(draw_legend = False):
...
width = .05,head_width = .23)
    ...
    if (draw_legend):
      if (planned_traj != None):
        plt.legend(['Planned Trajectory', 'Vehicle Trajectory',
 'Robot Position'],loc='best')
      else:
        plt.legend(['Vehicle Trajectory',
 'Robot Position'],loc='best')
...
if __name__ == '__main__':
  ...
  if (method == 1): # using the advanced robust method
    ...
    for i,w in enumerate(waypoints):
      ...
      planned_traj = bsplines.bspline(np.array([(x,y) for x,y,yaw in temp_waypoints]),np.array([yaw for x,y,yaw in temp_waypoints]),L*15,3,0)
      old_nearest_point_index = None
  ...
  print("total cost:total time{}\ntrajectory{}\nposes{}".format(cost[0],cost[1],cost[2]))

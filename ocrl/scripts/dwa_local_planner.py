...
class Config():
    ...
    def __init__(self):
        ...
        self.min_steering = 35.0 * math.pi / 180.0
        self.max_steering = 125.0 * math.pi / 180.0
...
def motion(x, u, dt):
    ...
    motion model
    """
    # x[0] = x, x[1] = 1, x[2] = yaw, x[3] = velocity, x[4] = turning rate
    x[2] += u[1] * dt
...
def main(gx=10, gy=10):
    ...
    # initial state [x(m), y(m), yaw(rad), v(m/s), omega(rad/s), rad]
    x = np.array([0.0, 0.0, math.pi / 8.0, 0.0, 0.0, 0.0, 90.0])
    ...
    while True:
        ...
        if show_animation:
            plt.cla()
            plt.plot(ptraj[:, 0], ptraj[:, 1], "-g") # green trajectory line
            plt.plot(x[0], x[1], "xr") # robot's position
            plt.plot(goal[0], goal[1], "xb") # goal position
            plt.plot(ob[:, 0], ob[:, 1], "ok") # obstacles
            plot_arrow(x[0], x[1], x[2]) # black arrow representing velocity
            plt.axis("equal")
            plt.grid(True)
            plt.pause(0.0001)
